...
def main(
    ...
    filename = f"ga_low_250305_{observation}{setup}_{int(t_max)}_{n_particles}_{smc_steps}_{seed}"
    jnp.savez(
        f"../results/{filename}.npz",
        hof=halloffame,
        final_pop=final_pop,
    )
    with open(f"../results/{filename}_losses.pickle", "wb") as handle:
        pickle.dump(ga_losses, handle)
    ####################################################################################
    ####################################################################################
    ################################# POSTHOC ANALYSIS #################################
    ####################################################################################
